...
export const SESSION_REFRESH_API_CONTRACT = frozen({
  loadSessions: { method: 'GET', path: '/sessions', nativeDiscovery: false, source: 'published-memory-snapshot' },
  ingestNativeSessions: { method: 'POST', path: '/sessions/refresh', nativeDiscovery: true, manualForceBypassesCompletedResultTtl: true },
  websocketSessions: { source: 'same-published-memory-snapshot-as-get' },
});
...
export const COMPLETED_RESULT_TTL_CONTRACT = frozen({
  defaultMs: 30_000, reusesPublishedSnapshot: true, successOnly: true, failureExtendsTtl: false, manualForceBypasses: true,
});
...
export const SESSION_FIELD_OWNERSHIP = frozen({
  nativeDerived: ['nativeIdentity','nativeEngineType','nativePath','discoveredCwd','nativeCandidateTitle','nativeCreatedAt','nativeObservedMtime','filesystemFingerprint','nativeAvailability','nativeStale','nativeScanError'],
  hubOwned: ['name','nameSource','agentType','status','model','effort','autoAllow','archived','engineRefs','lastEngine','pid','msgCount','queued','contextReset','hubMessages'],
...
export const SESSION_REFRESH_SLO = frozen({
  ...
  thousandSessionMetadata: { materializationMs: 20, httpJsonSerializationMs: 25, websocketJsonStringifyMs: 25, payloadBytesMustBeRecorded: true },
});

export function validateNativeScanResult(result) {
  const issues = [];
  if (!result || typeof result !== 'object' || Array.isArray(result)) return { ok: false, issues: ['scan result must be an object'] };
  for (const key of NATIVE_SCAN_RESULT_CONTRACT.requiredKeys) if (!Object.hasOwn(result, key)) issues.push(`missing ${key}`);
  if (typeof result.complete !== 'boolean') issues.push('complete must be boolean');
  for (const key of ['entries', 'seenPaths', 'errors', 'rootsScanned']) if (!Array.isArray(result[key])) issues.push(`${key} must be an array`);
  if (Array.isArray(result.errors)) {
    result.errors.forEach((error, index) => {
      if (!error || typeof error !== 'object' || Array.isArray(error)) { issues.push(`errors[${index}] must be an object`); return; }
      for (const key of NATIVE_SCAN_RESULT_CONTRACT.errorRequiredKeys) if (!Object.hasOwn(error, key)) issues.push(`errors[${index}] missing ${key}`);
      for (const key of NATIVE_SCAN_RESULT_CONTRACT.forbiddenErrorKeys) if (Object.hasOwn(error, key)) issues.push(`errors[${index}] exposes ${key}`);
    });
  }
