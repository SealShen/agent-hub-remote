...
function redmineInline(text, keyPrefix) {
  ...
  return parts.map((p, i) => p.href ? (
    <a
      className="md-a redmine-issue-link"
      key={`${keyPrefix}-${i}`}
      href={p.href}
      target="_blank"
  ...
  while (rest) {
    ...
    if (tok.startsWith('`')) {
      ...
      if (isWindowsFile) {
        ...
        out.push(
          <span className="md-file" key={key++} role="group" aria-label="本機檔案">
            <code>{codeText}</code>
            <span className="md-file-actions">
        ...
        out.push(<code className="md-code" key={key++}>{codeText}</code>);
      }
    } else if (tok.startsWith('**')) {
      out.push(<strong key={key++}>{mdInline(tok.slice(2, -2), opts)}</strong>);
    } else if (tok.startsWith('*')) {
      out.push(<em key={key++}>{mdInline(tok.slice(1, -1), opts)}</em>);
    } else if (tok.startsWith('[')) {
      const lm = tok.match(/\[([^\]]+)\]\(([^)\s]+)\)/);
      if (lm) {
        out.push(
          <a className="md-a" key={key++} href={lm[2]} target="_blank" rel="noreferrer">{mdInline(lm[1], { ...opts, enableRedmineLinks: false })}</a>
        );
      } else {
        pushPlain(tok);
      }
    } else {
      // bare URL — strip trailing punctuation that regex may have swallowed
      const href = tok.replace(/[.,;:!?。，；：、]+$/, '');
      out.push(
        <a className="md-a" key={key++} href={href} target="_blank" rel="noreferrer">{href}</a>
      );
...
function mdBlocks(text) {
  ...
  while (i < lines.length) {
    ...
    // code fence — CommonMark: closing fence must be >= length of opening
    // fence (same char), else a nested fence of fewer backticks (e.g. ```sql
    // inside an outer ````markdown block) would prematurely close the outer one.
    const fenceOpen = line.trimStart().match(/^(`{3,})(.*)$/);
    if (fenceOpen) {
      const fenceMarker = fenceOpen[1];
      const lang = fenceOpen[2].trim();
      const code = [];
      i++;
      const closeRe = new RegExp('^`{' + fenceMarker.length + ',}\\s*$');
      while (i < lines.length && !closeRe.test(lines[i].trimStart())) {
        code.push(lines[i]);
        i++;
      }
      i++;
    ...
    while (i < lines.length) {
      ...
      if (isTableRow(ln) && isTableSep(lines[i + 1] || '')) break;
      para.push(ln);
      i++;
    }
...
function MD({ text, enableRedmineLinks, sessionId }) {
  ...
  const inlineOpts = React.useMemo(
    () => ({ enableRedmineLinks: !!enableRedmineLinks, sessionId }),
    [enableRedmineLinks, sessionId],
  );
  return (
    <>
      {blocks.map((b, idx) => {
        ...
        if (b.kind === 'h') {
          const Tag = `h${b.level}`;
          return <Tag key={idx} className={`md-h md-h${b.level}`}>{mdInline(b.text, inlineOpts)}</Tag>;
        }
        if (b.kind === 'ulist') {
          return (
            <ul key={idx} className="md-list">
              {b.items.map((it, i) => <li key={i}>{mdInline(it, inlineOpts)}</li>)}
            </ul>
          );
        }
        if (b.kind === 'olist') {
        ...
        if (b.kind === 'quote') {
          return <div key={idx} className="md-quote">{mdInline(b.text, inlineOpts)}</div>;
        }
        if (b.kind === 'table') {
          return (
            <div className="md-table-wrap" key={idx}>
              <table className="md-table">
                <thead>
                  <tr>
                    {b.head.map((c, ci) => (
                      <th key={ci} style={{ textAlign: b.align[ci] || 'left' }}>{mdInline(c, inlineOpts)}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {b.rows.map((r, ri) => (
                    <tr key={ri}>
                      {r.map((c, ci) => (
                        <td key={ci} style={{ textAlign: b.align[ci] || 'left' }}>{mdInline(c, inlineOpts)}</td>
                      ))}
                    </tr>
                  ))}
                </tbody>
